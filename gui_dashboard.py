...
def launch_app(index):
    """
    Clears content and launches the app at `menu_items[index]`.
    """
    clear_content()
...
def open_gallery_gui():
    ...
    def refresh():
        ...
        for j, fr in enumerate(gallery_frames): fr.configure(border_width=2 if j==sel else 0)
...
def run_generation():
    ...
    def task():
        ...
        ir = img.resize((640,400)); ci=ctk.CTkImage(light_image=ir,dark_image=ir,size=(640,400))
...
def handle_keypress(line):
    ...
    if line.startswith('hotkey:'):
        k=int(line.split(':')[1])
        if k==1: draw_menu(); return
        if current_screen=='gallery' and k==2: show_sending(); gallery.send_to_inky(); return
        if current_screen=='images' and k==2: run_generation(); return
        if 3<=k<=11: launch_app(k-3); return
    if current_screen=='gallery':
        if line=='rotary:+1': gallery_update_callback[0]()
        elif line=='rotary:-1': gallery_update_callback[1]()


def macropad_listener():
    try: ser=serial.Serial('/dev/ttyACM1',115200,timeout=0.1)
    except Exception as e: print(f'? MacroPad conn fail: {e}'); return
    while not exit_requested:
        l=ser.readline().decode(errors='ignore').strip()
        if l: root.after(0,lambda ln=l: handle_keypress(ln))
        time.sleep(0.01)

if __name__=='__main__':
    draw_menu(); threading.Thread(target=macropad_listener,daemon=True).start(); root.mainloop()
